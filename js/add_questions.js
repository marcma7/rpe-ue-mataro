...
function obrirNovaPregunta(){
    ...
    preguntaEditant = {
        uuid:"",
        pregunta:"",
        tipus_pregunta: "ESCALA NUMÈRICA",
        opcions_resposta:""
    };
...
function obrirEditarPregunta(q){
    ...
    preguntaEditant = q;
    tipusPregunta = q.tipus_pregunta;

    if(tipusPregunta==="OPCIONS") opcionsPregunta = q.opcions_resposta.split("//").filter(x=>x);

    if(tipusPregunta==="ESCALA NUMÈRICA"){
        const nums = q.opcions_resposta.split("//").map(Number);
        minimPregunta = Math.min(...nums);
        maximPregunta = Math.max(...nums);
    }
...
function pintarFormPregunta(){
    ...
    document.getElementById("preguntaText").value = preguntaEditant.pregunta || "";
    document.getElementById("tipusPregunta").value = tipusPregunta;
    document.getElementById("minPregunta").value = minimPregunta;
    document.getElementById("maxPregunta").value = maximPregunta;
    ...
    divEscala.style.display = tipusPregunta === "ESCALA NUMÈRICA" ? "flex" : "none";
    divOpcions.style.display = tipusPregunta === "OPCIONS" ? "block" : "none";
...
function afegirOpcioPregunta(){
    ...
    const input = document.getElementById("novaOpcio");
    if(!input.value) return;

    opcionsPregunta.push(input.value);
    input.value="";
    pintarOpcionsPregunta();
}
...
function eliminarOpcioPregunta(opcio){
    opcionsPregunta = opcionsPregunta.filter(x=>x!==opcio);
    pintarOpcionsPregunta();
}
...
function pintarOpcionsPregunta(){
    ...
    const div = document.getElementById("llistaOpcions");
    if(!div) return;
    div.innerHTML="";
...
async function guardarPregunta(){
    ...
    const text = document.getElementById("preguntaText").value.trim();
    if(!text){
        alert("Escriu una pregunta");
        return;
    }
    ...
    const data = {
        ...
        tipus_pregunta: tipusPregunta,
        opcions_resposta: opcions,
        questionari_uuid: questionariActual.uuid
    };

...
async function getQuestionarisPerContestar(userUuid) {
    ...
    const url =
        SUPABASE_URL +
        "/rest/v1/questionaris_contestar" +
        "?user_uuid=eq." +
...
async function getUserTeamByUserUuid(userUuid) {
    ...
    const url =
        SUPABASE_URL +
        "/rest/v1/user_teams?user_uuid=eq." +
        encodeURIComponent(userUuid);
...
async function getPTPTByUserTeamUuids(uuids) {
    ...
    const url =
        SUPABASE_URL +
        "/rest/v1/player_team_practice_time" +
        "?player_team_uuid=in.(" +
        joined +
        ")" +
        "&select=*,practices(*)";
...
async function getRPEByUserUuid(userUuid) {
    ...
    const url =
        SUPABASE_URL +
        "/rest/v1/rpe_registers?player_uuid=eq." +
        encodeURIComponent(userUuid);
...
async function findUserByNameAndSurnameAndRole(name, surname, role) {
    ...
    const url =
        ...
        `&surname=eq.${encodeURIComponent(surname)}` +
...
async function updateSession(selectedDate){
    ...
    const practice =
        practices.find(
            x => x.practice_date === selectedDate
        );

    const ptpts =
        playerTeamPracticeTimes.filter(
    ...
    const practiceUuid =
        practices
        .filter(x=>x.practice_date===selectedDate)
        .map(x=>x.uuid);
        ...
        for(const uuid of playerUuid){
            ...
            const ptptUser =
                playerTeamPracticeTimes.filter(
                    ...
                    x.player_team_uuid===uTeam.uuid &&
                    practiceUuid.includes(
                        x.practice_uuid
                    )
...
async function getQuestionarisUser(questionariUuids, userUuids){
    ...
    const response =
        await fetch(
            ...
            `${SUPABASE_URL}/rest/v1/questionaris_contestar`+
            `?questionari_uuid=in.(${q})`+
            `&user_uuid=in.(${u})`,
...
async function getEpisodesByInjury(uuids){
    ...
    const url =
    `${SUPABASE_URL}/rest/v1/physio_episodes?injury_uuid=in.(${uuids.join(",")})`;
...
async function getVisitsByEpisodes(uuids){
    ...
    const url =
    `${SUPABASE_URL}/rest/v1/physio_visits?episode_uuid=in.(${uuids.join(",")})`;
...
async function getVisitsByDates(dates){
    ...
    const url =
    `${SUPABASE_URL}/rest/v1/physio_visits?date=in.(${dates.join(",")})`;
...
async function upsertPhysioHour(data){
    ...
    const url =
    `${SUPABASE_URL}/rest/v1/physio_visits?on_conflict=uuid`;
...
async function getEpisodesByUuid(uuids){
    ...
    const url =
    `${SUPABASE_URL}/rest/v1/physio_episodes?uuid=in.(${uuids.join(",")})`;
...
async function getInjuriesByUuid(uuids){
    ...
    const url =
    `${SUPABASE_URL}/rest/v1/injuries?uuid=in.(${uuids.join(",")})`;
...
async function closeEpisode(data){
    ...
    const url =
    `${SUPABASE_URL}/rest/v1/physio_episodes?on_conflict=uuid`;

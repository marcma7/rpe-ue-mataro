...
async function moureItemAmunt(index){
    ...
    const aux =
    valoracioItems[index-1];

    valoracioItems[index-1] =
    valoracioItems[index];
...
async function moureItemAvall(index){
    ...
    if(index===valoracioItems.length-1)
        return;
...
async function guardarOrdreItems(){
    ...
    await upsertValoracioItemOrder(
        updates
    );
...
document
.getElementById(
    "nouItemValoracioButton"
)
.onclick =
obrirNouValoracioItem;

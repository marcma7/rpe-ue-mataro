...
document
.getElementById("novaValoracioButton")
.onclick = obrirNovaValoracio;

document
.getElementById("tornarValoracioButton")
.onclick = ()=>{
...
async function loadGestValoracions(){
    ...
    const items =
    await getAllValoracionsItems();

    for(const v of valoracionsGestio){

        v.nItems =
        items.filter(
            x=>x.valoracio_uuid===v.uuid
        ).length;
...
async function eliminarValoracio(uuid){
    ...
    if(!confirm(
        "Segur que vols eliminar aquesta valoració?"
    ))
        return;
...
async function obrirValoracioItems(uuid){
    ...
    valoracioActualGestio =
    valoracionsGestio.find(
        x=>x.uuid===uuid
    );
...
async function obrirSeleccionarJugadorValoracio(){
    ...
    document.getElementById(
        "pantallaSeleccionarJugadorValoracio"
    ).style.display = "none";

    await obrirValoracio(

...
document
    .getElementById("modificarSessioButton")
    .addEventListener(
        "click",
        obrirModificarSessions
    );
...
document
    .getElementById("eliminarEquipButton")
    .addEventListener("click", eliminarEquip);

document
    .getElementById("afegirSessionsButton")
    .addEventListener("click", obrirSessions);
...
async function pickPlayers(teamUuid) {
    ...
    const userTeams =
        await getPlayersByTeam(teamUuid);

    const userUuids =
        userTeams.map(u => u.user_uuid);

    const users =
        await getUsersByUserTeam(userUuids);
...
function pintarJugadors(users) {
    ...
    const llista =
        document.getElementById("llistaJugadors");
...
async function eliminarJugador(user){
    ...
    if(!confirm(
        "Segur que vols eliminar aquest jugador de l'equip?"
    ))
        return;

    const teamUuid =
        document.getElementById("selectorTeams").value;

    const userTeam =
        await getUserTeamByTeamUuidAndUserUuid(
            teamUuid,
            user.uuid
        );
    ...
    const userTeams =
        await getUserTeamByUserUuid(user.uuid);
...
document
    .getElementById("afegirJugadorButton")
    .addEventListener("click", afegirJugador);


async function afegirJugador() {

    const nom =
        document.getElementById("nomJugador")
            .value.trim().toLowerCase();

    const cognom =
        document.getElementById("cognomJugador")
            .value.trim().toLowerCase();

    const rol =
        document.getElementById("rolJugador")
            .value;
    ...
    const users =
    await findUserByNameAndSurnameAndRole(
        nom,
        cognom,
        rol
    );
...
function createUserCode(name, surname, users) {
    ...
    const provisional =
        cleanName.charAt(0) + cleanSurname;

    const iguals =
        users.filter(u => {

            const code =
                removeAccents(u.name).charAt(0) +
                removeAccents(
                    u.surname.replaceAll(" ", "")
                );
...
async function eliminarEquip(){
    ...
    if(!confirm(
        `Segur que vols eliminar l'equip "${teamName}"?`
    ))
        return;

...
document
.getElementById("nouQuestionariButton")
.onclick = obrirNouQuestionari;
...
function obrirNouQuestionari(){
    ...
    document.getElementById(
        "descripcioQuestionari"
    ).value = "";

    document.getElementById(
        "nouQuestionariDialog"
    ).style.display = "flex";

}

document
.getElementById("cancelNouQuestionariButton")
.onclick = ()=>{
...
document
.getElementById("confirmNouQuestionariButton")
.onclick = async ()=>{
...
async function loadQuestionarisPendents(user, pendents) {
    ...
    const quest = await getQuestionarisFromUuid(
        questionaris.map(x => x.questionari_uuid)
    );

    for (const q of questionaris) {

        const info =
            quest.find(x => x.uuid === q.questionari_uuid);
...
async function loadQuestions() {
    ...
    questions = await getQuestionsFromQuestionari(
        questionaris[0].questionari_uuid
    );
...
function pintarQuestionari() {
    ...
    document.getElementById("questionariNom").textContent =
        questionaris[0].name;

    document.getElementById("questionariDescripcio").textContent =
        questionaris[0].description;
...
function crearEscala(bloc, question) {
    ...
    const numeros = question.opcions_resposta
    .split("//")
...
const opcions = question.opcions_resposta
    .split("//")
    .filter(opcio => opcio.trim() !== "");
...
async function eliminarQuestionari(uuid){
    ...
    if(!confirm(
        "Segur que vols eliminar aquest qüestionari?"
    ))
        return;

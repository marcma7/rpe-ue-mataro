...
function omplirSelectorDates() {
    ...
    const selector =
        document.getElementById("selectorData");
...
document
    .querySelectorAll(".rpeButton")
    .forEach(b => b.classList.remove("rpeSelected"));
...
function getRPEWeight(rpe, pf, train, game) {
    ...
    const pfSum =
        pf.reduce((sum, x) => sum + x.time, 0);

    const trainSum =
        train.reduce((sum, x) => sum + x.time, 0);

    const gameSum =
        game.reduce((sum, x) => sum + x.time, 0);
